...
def spredxcan(current_path,filename,snp,beta,z,p,a1,a2):
    ...
    code ='./SMulTiXcan.py --models_folder data/Models --models_name_filter en_Brain_.*db --models_name_pattern "en_Brain_(.*).db" --snp_covariance data/gtex_v8_expression_elastic_net_snp_smultixcan_covariance.txt.gz --metaxcan_folder results --metaxcan_filter ".*csv" --metaxcan_file_name_parse_pattern "en_Brain_(.*).csv" --gwas_file ".*gz" --snp_column SNP --non_effect_allele_column A2 --effect_allele_column A1 --beta_column beta --pvalue_column P --cutoff_condition_number 30 --verbosity 1 --throw --output '
